...
class prototype_explanation() :
    ...
    def visualizing(self, torch_data, data_type, title="Resulting Graph"):
        """
    ...
    def generate_prototype(self, label, k, n_components, n_iter, cluster_index, max_epochs = 40) :
        ...
        found_subgraphs = []
        print("Finished searching nearest graphs. Now searching maximum common subgraphs...")
        print(pos)
        for n1, n2 in combin :
            G1 = self.data[n1]
            G2 = self.data[n2]
        ...
        for n1, n2 in second_combin :
            ...
            final_graphs.append(self.shifted_graph_to_torch(
                G = subg[0], features = subg[1]))
        self.visualizing(final_graphs[0], data_type = self.data_type,title = "Prototype {0} cluster".format(cluster_index))
        #for i, graph in enumerate(final_graphs) :
        #    self.visualizing(graph, title = "Prototype {0} of {1} cluster".format(i, cluster_index))
        return final_graphs