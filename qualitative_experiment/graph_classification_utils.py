...
def ba_house_training(model, data_module, test_data, device, epochs = 300) :
    ...
    for epoch in range(epochs):
        ...
        if epoch % 5 == 0 :
            ...
            print("Epoch : {0} / Training Loss : {1} / Test Acc : {2}".format(epoch, loss.to("cpu").item(), test_result))
        """
    return model
...
def solubility_training(model, data_module, test_data, device, epochs = 300) :
    ...
    for epoch in range(epochs):
        ...
        """
        if epoch % 10 == 0:
            print("Current Epoch {0} / Training Loss {1} / Test ACC {2}".format(epoch, epoch_loss,
                                                                                solubility_class_evaluator(model, test_data, device)))
        """
    return model