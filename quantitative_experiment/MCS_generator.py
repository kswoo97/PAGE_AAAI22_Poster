...
class MCS_generation():
        ...
        for epoch in range(1, 1 + max_epochs):
            ...
            for i in range(min(len(NB1), len(NB2))):
                ...
                if True:
                    ...
                    # plt.imshow( self.Y.detach().numpy(), cmap=cm.Reds )
                    # plt.clim(0,1)
                    # plt.colorbar()
                    # plt.show()
                    break
    ...
    def select_subgraph(self, max_index=5, max_epochs=1000, decay=1.5, threshold=0.001):
        ...
        for m in range(1, max_index + 1):
            ...
            # draw_subgraph(self.G1,node1, tu_node_index,tu_color_list,name=f'order {m} graph {self.G1}')
            # draw_subgraph(self.G2,node2, tu_node_index,tu_color_list, name=f'order {m} graph {self.G2}')
        ...
        node1 = node1s[M]
        node2 = node2s[M]
        # draw_subgraph(self.G1,node1, tu_node_index,tu_color_list,name=f'last order {m} graph {self.G1}')
        # draw_subgraph(self.G2,node2, tu_node_index,tu_color_list, name=f'last order {m} graph {self.G2}')
        return node1, node2
